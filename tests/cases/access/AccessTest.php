...
class AccessTest extends \lithium\test\Unit {
	...
	public function testSimpleFieldRule() {
		...
		$this->assertFalse(Access::check($user, array('is_owner' => $resource), false));
		$user->id = 99;
		$this->assertTrue(Access::check($user, ['is_owner' => [$resource, $resource2]], false));
		$user->id = 100;
		$this->assertTrue(Access::check($user, ['is_owner' => [$resource, $resource2]], false));
		$user->id = 5;
		$this->assertFalse(Access::check($user, ['is_owner' => [$resource, $resource2]], false));
		$user->id = null;
		$this->assertFalse(Access::check($user, ['is_owner' => [$resource, $resource2]], false));
	}
