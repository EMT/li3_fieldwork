...
class MarkdownParser implements IMarkdownParser {
	...
	function doIframes($text) {
	#
	# Turn Markdown iframe shortcuts into <iframe> tags.
	#
		#
		# Handle inline iframes:  ![[url]]
		# Don't forget: encode * and _
		#
		$text = preg_replace_callback('{
			(				# wrap whole match in $1
			  \[\[
				('.$this->nested_brackets_re.')		# url = $2
			  \]\]
			)
			}xs',
			array(&$this, '_doIframes_inline_callback'), $text);

		return $text;
	}
	function _doIframes_inline_callback($matches) {
		$whole_match	= $matches[1];
		$url			= $matches[2];

		$url = $this->encodeAttribute($url);
		$result = "<iframe src=\"$url\"></iframe>";
